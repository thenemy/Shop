...
<div x-data="{ open: false }" class="cursor-pointer">
    <div
        class="@if($item->isCurrentRoute()) bg-blue-300 rounded-md @endif flex flex-row items-center h-12
        transform hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800">
        <div>
            <button @click="open = ! open">
        <span class="inline-flex items-center justify-center w-40 text-lg text-gray-400"><i
    ...
    @foreach($item->sublist as $sublist)
        <div x-show="open" @click.outside="open = false" class="@if($item->isCurrentRoute()) bg-blue-400 rounded-md @endif">
            <div
                class="@if($item->isCurrentRoute()) bg-blue-300 rounded-md @endif h-7 pl-16 transform text-sm font-medium hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800">
